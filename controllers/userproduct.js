...
module.exports.buyfromcart = async function (req, res) {
    services.userproduct.buyfromcart({
        "_id": req.session.passport.user._id
    }, req, res);
}

module.exports.deletefromcart = async function (req, res, next) {
    var details = await services.userproduct.deletefromcart({
        "_id": req.session.passport.user._id
    }, req, res);
}

module.exports.cleancart = async function (req, res, next) {
    var details = await services.userproduct.cleancart({
        "_id": req.session.passport.user._id
    }, req, res);
}

module.exports.addToCart = async function (req, res, next) {
    var details = await services.userproduct.addToCart({
        "_id": req.session.passport.user._id
    }, req, res);
}